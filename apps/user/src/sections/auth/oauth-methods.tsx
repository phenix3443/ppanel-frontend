...
// 【图标与标签只有一份】OAUTH_PROVIDER_META 同时被 bind、oauth、
// third-party-accounts 三处使用；在这里再内联一份映射迟早会和它们分叉。
type OAuthMethod = keyof typeof OAUTH_PROVIDER_META;

function isOAuthMethod(method: string): method is OAuthMethod {
  return Object.hasOwn(OAUTH_PROVIDER_META, method);
}
          ...
          {OAUTH_METHODS.map((method) => (
            ...
            >
              <Icon
                ...
                icon={getOAuthProviderMeta(method).icon}
              />
            </Button>
          ))}
        </div>
